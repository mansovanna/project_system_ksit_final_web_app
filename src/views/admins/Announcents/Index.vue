...
const defineData = ref({ title: '', body: '' })

const openFormcreate = () => {
  isOpenFromCreate.value = !isOpenFromCreate.value
}
const openFormUpdate = (id: number | null, title: string, body: string) => {
  if (id != null) {
    defineData.value.title = title
    defineData.value.body = body
...
const closeAndClear = () => {
  defineData.value.title = ''
  defineData.value.body = ''
  isOpenFromUpdate.value = false
}
</script>
<template>
  <AdminLayouts title="User Register New">
    <FormUpdate
      @close="closeAndClear"
      v-if="isOpenFromUpdate"
      :body="defineData.body"
      :title="defineData.title"
    />
    <!-- Block Alert Select status -->
    <FormCreate v-if="isOpenFromCreate" @close="openFormcreate" />
    <div
      v-if="isOpen"
        ...
        <div class="w-full flex justify-between items-center">
          ...
          <div class="flex w-1/4 justify-end items-center gap-6">
            ...
            <button
              ...
              class="px-4 py-2 bg-blue-700 text-white rounded-md hover:bg-blue-600 font-Kantumruy text-nowrap"
            >
              Add New
            </button>
          </div>
          ...
          <table class="w-full">
            <thead>
              ...
              >
                ...
                <td class="px-1 py-1.5 text-left dark:text-slate-300">Title</td>
                <td class="px-3 py-1.5 text-center dark:text-slate-300">Body</td>
                <td class="px-3 py-1.5 text-center dark:text-slate-300">Admmin</td>
                <td class="px-3 py-1.5 text-center dark:text-slate-300">Created_At</td>
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-1 py-1.5 font-Kantumruy text-slate-500 dark:text-slate-300 text-left">
                  <p class="text-wrap line-clamp-1">
                    Lorem ipsum dolor, sit amet consectetur adipisicing elit. Optio odit iste
                    consequatur. Magnam quae delectus error accusamus sapiente illum rerum neque?
                    Doloremque omnis nisi quo dolores laboriosam? Cupiditate, earum vitae?
                  </p>
                </td>
                <td
                ...
                <td class="py-1.5 text-center">
                  ...
                  <div class="flex justify-center items-center gap-2">
                    <button
                      @click="openFormUpdate(1, 'App', 'Programmers')"
